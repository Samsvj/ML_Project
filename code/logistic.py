...
class Logistic(object):
    ...
    #evaluate the probability of belonging to class 1 P(C|phi_n) = sigmoid(w.T * phi_n)
    def evaluate(self,phi_n):
        return sig(np.dot(self.weights, phi_n.T))

    #update the weights using gradient descent until convergence phi is a matrix of feature vectors, one row for each datum labels is a 1 X N array of binary labels
    ...
    #predict test data: return 1 if the datum is more probable to be in class 1  and 0 if the datum is more probable to be in class 0
